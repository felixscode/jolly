<script lang="ts">
	...
	let availableForDownload = $derived(
		AVAILABLE_MODELS.filter(
			(m) => !settings.downloadedModels.some((d) => d.id === m.id)
		)
	);

	$effect(() => {
		if (availableForDownload.length > 0 && !availableForDownload.some(m => m.id === selectedModelId)) {
...
<!-- Panel -->
<div
	class="fixed top-0 right-0 z-50 flex h-full w-[70%] max-w-[560px] flex-col bg-white shadow-xl transition-transform duration-200 ease-out dark:bg-gray-800 {open ? 'translate-x-0' : 'translate-x-full'}"
>
	<!-- Header -->
		...
		<section>
			...
			<div class="mt-2 flex gap-2">
				...
				<button
					...
					class="rounded-md bg-[#241e4e] px-4 py-2 text-sm font-medium text-white hover:bg-[#1a1639] transition-colors"
				>
					{keySaved ? 'Saved!' : 'Save'}
				</button>
			</div>
		...
		<section>
			<div class="flex items-center justify-between">
				...
				<button
					...
					class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {settings.darkModeOverride ? 'bg-[#241e4e]' : 'bg-gray-300 dark:bg-gray-600'}"
					role="switch"
					aria-checked={settings.darkModeOverride}
				>
					<span
						class="inline-block h-4 w-4 rounded-full bg-white transition-transform {settings.darkModeOverride ? 'translate-x-6' : 'translate-x-1'}"
					></span>
				</button>
			</div>
			...
			{#if availableForDownload.length > 0}
				<div class="mt-2 flex gap-2">
					...
					<button
						...
						disabled={!!settings.downloadProgress}
						class="rounded-md bg-[#241e4e] px-4 py-2 text-sm font-medium text-white hover:bg-[#1a1639] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
					>
						Download
					</button>
				</div>
			...
			{:else}
				<div class="mt-2 space-y-2">
					{#each settings.downloadedModels as model}
						<button
							onclick={() => settings.setActiveModel(
								settings.activeModelId === model.id ? null : model.id
							)}
